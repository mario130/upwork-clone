...
module.exports.authenticate = (req, resp, next) => {
  passport.authenticate("local", (err, user, info) => {
    ...
    // registered user
    else if (user) return resp.status(200).json({ token: user.generateJwt(),userType:user.userType, userName:user.userName,firstName:user.firstName, lastName: user.lastName, _id:user._id });
